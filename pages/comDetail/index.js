...
Page({
    ...
    onLoad: function(options) {
        ...
        if (options.referer) {
            ...
            wx.reportAnalytics('detail_referer', {
                ue: source
            });
        }
    },
    ...
    toMap: function() {
        ...
        if (this.data.store && this.data.store.lat && this.data.store.lng) {
            wx.navigateTo({
                url: '/pages/map/index?lat=' + this.data.store.lat + '&lng=' + this.data.store.lng
            });
        }
    },
    toBuy: function() {
    ...
    okSelect: function() {
        if (this.data.defaultSku.stock == 0) {
            errDialog("此规格库存不足");
            return
        }
        var point = this.data.productInfo.point == null ? 0 : this.data.productInfo.point;
    ...
    selectType: function(e) {
        if (e.currentTarget.dataset.stock > 0) {
            this.setData({
                ...
                curSkuMajorId: e.currentTarget.dataset.id
            });
            this.setData({
                defaultSku: getObjById(this.data.productSkus, this.data.curSkuId)
            });
        }
    },
    ...
    onShareAppMessage: function(res) {
        ...
        var share = {
            ...
            path: '/pages/login/index?pagetype=1&inner=1&pid=' + that.data.productId + '&storeid=' + that.data.storeId + '&invitecode=' + (this.data.invitecode ? this.data.invitecode : wx.getStorageSync('inviteCode')),
